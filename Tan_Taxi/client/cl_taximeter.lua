...
function GetTaxi(vehicle)
    if not DecorExistOn(vehicle, fareDecor) or not DecorExistOn(vehicle, extrasDecor) then
        return fare, extras
    end
...
function isTaxi(vehicle)
    for _, veh in pairs(Config.TaxiVehicles) do
    ...
    while true do
        ...
        if vehicle ~= 0 and isTaxi(vehicle) then
            ...
            elseif seat == ped and not isDriver then
                isDriver = true
                SendNUIMessage({ type = "updateDriver", driver = isDriver })
